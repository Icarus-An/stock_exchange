...
public class QuotationService {
    ...
    void processMessage(TickMessage message) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("process ticks: sequenceId = {}, {} ticks...", message.sequenceId, message.ticks.size());
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("generated ticks data: {}", ticksData);
        }
        ...
        if (!tickOk.booleanValue()) {
            log.warn("ticks are ignored by Redis.");
            return;
        }
        // 保存Tick至数据库:
        ...
        log.info("returned created bars: " + strCreatedBars);
        // 将Redis返回的K线保存至数据库:
