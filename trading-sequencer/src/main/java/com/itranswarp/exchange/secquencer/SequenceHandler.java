...
public class SequenceHandler {
    ...
    public List<AbstractEvent> sequenceMessages(final MessageTypes messageTypes, final AtomicLong sequence,
        ...
        if (t < this.lastTimestamp) {
            log.warn("[Sequence] current time {} is turned back from {}!", t, this.lastTimestamp);
        } else {
        ...
        for (AbstractEvent message : messages) {
            ...
            if (uniqueId != null) {
                ...
                log.info("unique event {} sequenced.", uniqueId);
            }
    ...
    public long getMaxSequenceId() {
        ...
        if (last == null) {
            log.info("no max sequenceId found. set max sequenceId = 0.");
            return 0;
        }
