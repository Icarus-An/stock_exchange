...
public class TradingEngineService {
        ...
        for (;;) {
            ...
            if (!msgs.isEmpty()) {
                if (log.isDebugEnabled()) {
                    log.debug("send {} tick messages...", msgs.size());
                }
    ...
    private void runNotifyThread() {
        log.info("start publish notify to redis...");
        for (;;) {
    ...
    private void runApiResultThread() {
        log.info("start publish api result to redis...");
        for (;;) {
    ...
    private void runOrderBookThread() {
        log.info("start update orderbook snapshot to redis...");
        ...
        for (;;) {
            ...
            if (orderBook != null && orderBook.sequenceId > lastSequenceId) {
                if (log.isDebugEnabled()) {
                    log.debug("update orderbook snapshot at sequence id {}...", orderBook.sequenceId);
                }
    ...
    private void runDbThread() {
        log.info("start batch insert to db...");
        for (;;) {
            try {
                saveToDb();
            } catch (InterruptedException e) {
    ...
    private void saveToDb() throws InterruptedException {
        if (!matchQueue.isEmpty()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("batch insert {} match details...", batch.size());
            }
        ...
        if (!orderQueue.isEmpty()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("batch insert {} orders...", batch.size());
            }
            this.storeService.insertIgnore(batch);
        }
        if (matchQueue.isEmpty()) {
    ...
    public void processEvent(AbstractEvent event) {
        ...
        if (event.previousId > this.lastSequenceId) {
            log.warn("event lost: expected previous id {} but actual {} for event {}", this.lastSequenceId,
                    event.previousId, event);
            List<AbstractEvent> events = this.storeService.loadEventsFromDb(this.lastSequenceId);
            if (events.isEmpty()) {
                log.error("cannot load lost event from db.");
                panic();
                return;
            }
        ...
        if (event.previousId != lastSequenceId) {
            log.error("bad event: expected previous id {} but actual {} for event: {}", this.lastSequenceId,
                    event.previousId, event);
            panic();
            return;
        }
        if (log.isDebugEnabled()) {
            log.debug("process event {} -> {}: {}...", this.lastSequenceId, event.sequenceId, event);
        }
        try {
            if (event instanceof OrderRequestEvent) {
                ...
                log.error("unable to process event type: {}", event.getClass().getName());
                panic();
                return;
            }
        } catch (Exception e) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("set last processed sequence id: {}...", this.lastSequenceId);
        }
        if (debugMode) {
            this.validate();
            this.debug();
    ...
    void require(boolean condition, String errorMessage) {
        if (!condition) {
            log.error("validate failed: {}", errorMessage);
            panic();
        }
    }
}