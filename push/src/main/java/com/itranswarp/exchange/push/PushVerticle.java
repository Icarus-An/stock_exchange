...
public class PushVerticle extends AbstractVerticle {
    ...
    public void start() {
        ...
        router.get("/notification").handler(requestHandler -> {
            ...
            log.info("parse user id from token: {}", userId);
            // 将连接升级到WebSocket:
    ...
    void initWebSocket(ServerWebSocket websocket, Long userId) {
        ...
        log.info("websocket accept userId: " + userId + ", handlerId: " + handlerId);
        // handle text message:
        websocket.textMessageHandler(str -> {
            log.info("text message: " + str);
        });
        ...
        websocket.closeHandler(e -> {
            ...
            log.info("websocket closed: " + handlerId);
        });
    ...
    public void broadcast(String text) {
        ...
        try {
            ...
            log.error("invalid message format: {}", text);
            return;
        }
        if (message.userId == null) {
            if (log.isInfoEnabled()) {
                log.info("try broadcast message to all: {}", text);
            }
            // broadcast to all:
            ...
            if (log.isInfoEnabled()) {
                log.info("try broadcast message to user {}: {}", message.userId, text);
            }
    ...
    void subscribeUser(String handlerId, Long userId) {
        ...
        set.add(handlerId);
        log.info("subscribe user {} {} ok.", userId, handlerId);
    }
    ...
    void unsubscribeUser(String handlerId, Long userId) {
        ...
        if (set != null) {
            ...
            if (set.isEmpty()) {
                ...
                log.info("unsubscribe user {} {} ok: cleared.", userId, handlerId);
            } else {
                log.info("unsubscribe user {} {} ok: but still others online.", userId, handlerId);
            }
        }
    }
}